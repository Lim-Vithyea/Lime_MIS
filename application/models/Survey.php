...
class Survey extends LSActiveRecord implements PermissionInterface
{
    ...
    public function search($options = [])
    {
        ...
        //This line set individual user UI, which only let the creator of survey see their survey

        // if(!Permission::model()->hasGlobalPermission('superadmin','read')){
        //     $criteria->compare('t.owner_id',Yii::app()->user->id);
        // }

        //This line let only the group of user see their group survey
        ...
        //     if (!empty($groupIds)) {
        //         $userIdsInGroup = Yii::app()->db->createCommand()
        //             ->select('uid')
        //             ->from('{{user_in_groups}}')
        //             ->where(['in', 'ugid', $groupIds])
        //             ->queryColumn();

        //         $criteria->addInCondition('t.owner_id', $userIdsInGroup);
        //     } else {
        //         $criteria->compare('t.owner_id', $userId);
        //     }
        // }
        if (!Permission::model()->hasGlobalPermission('superadmin', 'read')) {
        $userId = Yii::app()->user->id;
        ...
        if (!empty($groupIds)) {
            $peerUserIds = Yii::app()->db->createCommand()
                ->selectDistinct('uid')
                ->from('{{user_in_groups}}')
                ->where(['in', 'ugid', $groupIds])
                ->queryColumn();

            $criteria->addInCondition('t.owner_id', $peerUserIds);
        } else {
            // If user is not in any group, show only their own surveys
            $criteria->compare('t.owner_id', $userId);
        }
    }
